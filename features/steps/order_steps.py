"""Step definitions for order pricing scenarios."""
from behave import given, when, then
from src.entities import Product, OrderItem
from src.order_service import OrderService


@given('no promotions are applied')
def step_no_promotions(context):
    """Set up order service with no promotions."""
    context.order_service = OrderService()


@given('the threshold discount promotion is configured')
def step_threshold_discount_configured(context):
    """Set up order service with threshold discount promotion."""
    if not hasattr(context, 'order_service') or context.order_service is None:
        context.order_service = OrderService()
    # Store threshold discount configuration
    for row in context.table:
        threshold = float(row['threshold'])
        discount = float(row['discount'])
        context.order_service.set_threshold_discount(threshold, discount)


@given('the buy one get one promotion for cosmetics is active')
def step_bogo_cosmetics_active(context):
    """Set up order service with buy one get one promotion for cosmetics."""
    if not hasattr(context, 'order_service') or context.order_service is None:
        context.order_service = OrderService()
    context.order_service.set_buy_one_get_one_cosmetics(True)


@given('the Double 11 promotion is active')
def step_double11_active(context):
    """Set up order service with Double 11 promotion."""
    if not hasattr(context, 'order_service') or context.order_service is None:
        context.order_service = OrderService()


@given('the bulk discount rule for Double 11 is')
def step_double11_bulk_discount_rule(context):
    """Configure bulk discount rule for Double 11."""
    if not hasattr(context, 'order_service') or context.order_service is None:
        context.order_service = OrderService()
    for row in context.table:
        group_size = int(row['groupSize'])
        discount_rate_str = row['discountRate'].rstrip('%')
        discount_rate = float(discount_rate_str)
        context.order_service.set_double11_bulk_discount(group_size, discount_rate)


@when('a customer places an order with')
def step_place_order(context):
    """Process an order with given items."""
    items = []
    for row in context.table:
        product = Product(
            name=row['productName'],
            unit_price=float(row['unitPrice']),
            category=row.get('category', '')
        )
        order_item = OrderItem(product=product, quantity=int(row['quantity']))
        items.append(order_item)
    
    context.order = context.order_service.checkout(items)


@then('the order summary should be')
def step_verify_order_summary(context):
    """Verify the order summary amounts."""
    for row in context.table:
        if 'totalAmount' in row.headings:
            expected_total = float(row['totalAmount'])
            assert context.order.total_amount == expected_total, \
                f"Expected total amount {expected_total}, got {context.order.total_amount}"
        
        if 'originalAmount' in row.headings:
            expected_original = float(row['originalAmount'])
            assert context.order.original_amount == expected_original, \
                f"Expected original amount {expected_original}, got {context.order.original_amount}"
        
        if 'discount' in row.headings:
            expected_discount = float(row['discount'])
            assert context.order.discount == expected_discount, \
                f"Expected discount {expected_discount}, got {context.order.discount}"


@then('the customer should receive')
def step_verify_received_items(context):
    """Verify the items customer will receive."""
    for row in context.table:
        product_name = row['productName']
        expected_quantity = int(row['quantity'])
        actual_quantity = context.order.received_items.get(product_name, 0)
        
        assert actual_quantity == expected_quantity, \
            f"Expected {expected_quantity} of {product_name}, got {actual_quantity}"
